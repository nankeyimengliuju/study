...
<div class="page-header">
    当前位置：<span class="text-primary">已售罄</span>
</div>
<div class="page-content">
    ...
    <table class="table  table-responsive">
        ...
        <tr>
            ...
            <td>
                <a href="javascript:;" data-toggle="ajaxEdit" data-href="<?php  echo webUrl('goods.change',array('type'=>total,'id'=>$value['goods_id']));?>" style="display: inline;color: red;"><?php  echo $value['total'];?></a>
            </td>
            <!--<td>-->
                <!--<a><?php  if($value['total']<10) { ?><font style="color: red"><?php  } ?><?php  echo $value['total'];?></font></a>-->
            <!--</td>-->
            <td><a><?php  echo $value['act_times'];?></a></td>
            <td><a title="<?php  $day = intval((strtotime($value['end_time'])-time())/3600/24);$hour = intval((strtotime($value['end_time'])-time()-$day*3600*24)/3600);?>距离活动结束剩余:<?php  if($day > 0) { ?><?php  echo $day;?>天<?php  } else { ?><?php  echo $hour;?>小时<?php  } ?>"><?php  if($day > 0) { ?><?php  echo $day;?>天<?php  } else { ?><?php  echo $hour;?>小时<?php  } ?></a></td>

            <td  style="overflow:visible;position:relative">
                
                <?php if(cv('bargain.react')) { ?><a  class='btn btn-default btn-sm btn-op btn-operation' href="<?php  echo webUrl('bargain/react',array('actid'=>$value['id']));?>" title="编辑">
                <span data-toggle="tooltip" data-placement="top" title="" data-original-title="编辑">
                     <i class="icow icow-bianji2"></i>
                 </span>
            </a><?php  } ?>
                <?php if(cv('bargain.huishou')) { ?><a  class='btn btn-default btn-sm btn-op btn-operation' data-toggle="ajaxRemove" href="<?php  echo webUrl('bargain/huishou',array('id'=>$value['id']));?>" data-confirm='确认删除此商品？'>
